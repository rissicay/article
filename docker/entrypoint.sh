...
waitForElasticsearch() {
  echo -n "Waiting on elasticsearch(${ESHOST}:${ESPORT}) to start."
  for ((i=1;i<=30;i++))
  do
    health=$(curl --silent -u elastic:changeme "http://${ESHOST}:${ESPORT}/_cat/health" | awk '{print $4}')
